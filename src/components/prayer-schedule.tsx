...
export default function PrayerSchedule() {
  ...
  const todayKey = now
    ? `${now.getFullYear()}-${formatTwo(now.getMonth() + 1)}-${formatTwo(now.getDate())}`
    : "";
  ...
  const loadPrayerTimesAt = (latitude: number, longitude: number) =>
    fetchPrayerTimes(
      latitude,
      longitude,
      storedLocation?.kabupaten || storedLocation?.city || undefined,
    );
  ...
  }, [coords.latitude, coords.longitude, todayKey, storedLocation?.city, storedLocation?.kabupaten]);
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                <p className="truncate text-sm font-semibold text-slate-800">
                  {snapshot?.source === "myquran" && snapshot.location?.kabko
                    ? `${snapshot.location.kabko}, ${snapshot.location.prov ?? ""}`
                    : activeLocation
                      ? `${activeLocation.city}${activeLocation.district ? `, ${activeLocation.district}` : ""}${activeLocation.kabupaten && activeLocation.kabupaten !== activeLocation.city ? ` · ${activeLocation.kabupaten}` : ""}`
        ...
        <section className="rounded-3xl bg-white shadow-sm ring-1 ring-slate-100">
          <div className="flex flex-wrap items-center justify-between gap-2 px-6 py-5">
            ...
            <span className="text-[11px] font-medium text-slate-400">
              ...
              {snapshot?.source === "aladhan" ? "Aladhan" : "Kemenag RI"}
            </span>
          </div>
          <div className="flex flex-col px-3 pb-3">
            {times.map((prayer) => {
              ...
              } else if (prayer.key === "subuh" || prayer.key === "terbit" || prayer.key === "dhuha") {
